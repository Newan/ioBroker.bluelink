...
nextUri += `${nextUri.includes('?') ? '&' : '?'}country=${country}`;
    ...
    // Password RSA-ciphertext is hex-encoded (matches the Python hyundai_kia_connect_api
    // reference) - live-tested across many logins, base64 (which matches our legacy flow)
    // never once worked here and only cost an extra round-trip, so it's gone.
    ...
    if (certsResp.statusCode !== 200) {
        throw new Error(`Certs endpoint returned ${certsResp.statusCode}: ${certsResp.body.slice(0, 200)}`);
    }
    const jwk = JSON.parse(certsResp.body).retValue;
    if (!jwk || !jwk.kid) {
        throw new Error(`No JWK in certs response: ${certsResp.body.slice(0, 200)}`);
    }
    const encPw = encryptPassword(jwk, password, 'hex');
    info(`[tokenManager] CCI Step 3: POST https://${host}/auth/account/signin (kid=${jwk.kid})`);
    const signinBody = new URLSearchParams({
        client_id: cciCfg.clientId,
        encryptedPassword: 'true',
        password: encPw,
    ...
    jar.ingest(step3.headers);
    const signinLocation = step3.headers['location'] || '';
    info(`[tokenManager] CCI Step 3: HTTP ${step3.statusCode}, location(full)=${signinLocation}`);
    if (step3.statusCode !== 302) {
        throw new Error(`Signin failed: HTTP ${step3.statusCode} — ${step3.body.slice(0, 300)}. Check username and password.`);
    }
    ...
    if (code) {
        info(`[tokenManager] CCI Step 3: Authorization code obtained (${code.length} chars)`);
    } else if (locUrl.searchParams.has('error')) {
    ...
    } else if (signinLocation.includes('/web/v1/user/authorization')) {
        throw new Error('Account consent is required. Please log in via a browser once to accept the terms, then retry.');
    } else {
        throw new Error(`Signin returned to login page or unexpected redirect: ${signinLocation}`);
    }
