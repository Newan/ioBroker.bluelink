 ...
 * @returns {Promise<{ statusCode?: number, headers: Record<string, string | string[] | undefined>, body: string }>} Resolves with response details
 */
 ...
 *
 * @param {object} jwk JWK certs object
 ...
 * @param {(msg: string) => void} [log] optional logger function
 * @returns {Promise<{ refreshToken: string, accessToken: string, expiresAt: string }>} Token object
 */
...
throw new Error(`No JWK in certs response: ${step2.body.slice(0, 200)}`);
    ...
    const rawLoc = step3.headers['location'];
