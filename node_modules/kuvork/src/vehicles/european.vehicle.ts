...
export default class EuropeanVehicle extends Vehicle {
  ...
  public async monthlyReport(
    ...
    try {
      const response = await got(
        `${EU_BASE_URL[this.userConfig.brand]}/api/v2/spa/vehicles/${this.vehicleConfig.id}/monthlyreport`,
        {
          method: 'POST',
          headers: {
            'Authorization': this.controller.session.controlToken,
  ...
  public async tripInfo(
    ...
    try {
      const response = await got(
        `${EU_BASE_URL[this.userConfig.brand]}/api/v1/spa/vehicles/${this.vehicleConfig.id}/tripinfo`,
        {
          method: 'POST',
          headers: {
  ...
  public async getChargeTargets(): Promise<DeepPartial<VehicleTargetSOC>[] | undefined> {
    ...
    try {
      const response = await got(
        `${EU_BASE_URL[this.userConfig.brand]}/api/v2/spa/vehicles/${this.vehicleConfig.id}/charge/target`,
        {
          method: 'GET',
          headers: {
            'Authorization': this.controller.session.controlToken,
